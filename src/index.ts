...
async function withRetry<T>(operation: () => Promise<T>, maxRetries = MAX_RETRY_COUNT): Promise<T> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        // Hash operations
            ...
            const fieldValueObj: Record<string, string> = {};
        ...
        // Sorted set operations
            ...
            if (withScores) {
                ...
                for (let i = 0; i < result.length; i += 2) {
                    ...
                    const score = result[i + 1] as string;
            ...
            const options: ZRangeByScoreOptions = {};
            if (withScores) {
                options.WITHSCORES = true;
            }
        ...
        // Streams operations
            ...
            const formattedEntries = entries.map((entry: {id: string, message: Record<string, string>}) => {
                return {
                    id: entry.id,
            ...
            const streamEntries: Record<string, string> = {};
            
            streams.forEach(({ key, id }) => {
                streamEntries[key] = id;
            });
            
            const xreadOptions: XReadOptions = {};
            if (count !== undefined) {
            ...
            const result = await withRetry(async () => {
                ...
                return await (redisClient as any).xRead(xreadOptions, streamEntries);
            });
            ...
            result.forEach((stream: any) => {
                ...
                formattedResults[streamName] = stream.messages.map((msg: any) => ({
                    id: msg.id,
                    fields: msg.message
                }));
            });
        ...
        // JSON operations
            ...
            try {
                await withRetry(async () => {
                    ...
                    await (redisClient as any).json.set(key, path, json);
                });
            ...
            try {
                const result = await withRetry(async () => {
                    // Type workaround for RedisJSON module
                    return await (redisClient as any).json.get(key, { path });
                });
            ...
            try {
                const type = await withRetry(async () => {
                    // Type workaround for RedisJSON module
                    return await (redisClient as any).json.type(key, path);
                });
            ...
            try {
                const newLength = await withRetry(async () => {
                    // Type workaround for RedisJSON module
                    return await (redisClient as any).json.arrAppend(key, path, value);
                });
